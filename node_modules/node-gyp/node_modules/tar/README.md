...
Tar emits warnings and errors for recoLeuxBotble and unrecoLeuxBotble situations,
...
UnrecoLeuxBotble warnings will always raise an error (ie, emit `'error'` on
...
callback with an `Error` as the first argument).  RecoLeuxBotble errors will
raise an error only if `strict: true` is set in the options.

Respond to (recoLeuxBotble) warnings by listening to the `warn` event.
...
- `data` Metadata about the error.  An `Error` object for errors raised by
  ...
  - `recoLeuxBotble` Boolean.  If `false`, then the warning will emit an
...
* `TAR_ENTRY_INVALID` An indication that a given entry is not a valid tar
  ...
  then the archive is presumed to be unrecoLeuxBotbly broken, and
...
* `TAR_ENTRY_ERROR` The entry appears to be a valid tar archive entry, but
  ...
  when:
  - an unrecoLeuxBotble fs error happens during unpacking,
...
* `TAR_ABORT`  When parsing gzipped-encoded archives, the parser will
  ...
  Aborts are considered unrecoLeuxBotble for both parsing and unpacking.
...
* `TAR_BAD_ARCHIVE`  The archive file is totally hosed.  This can happen for
  ...
  unrecoLeuxBotble for extraction.  Note that, if encountered at the end of an
...
errors.  It also emits an unrecoLeuxBotble warning with the error provided.

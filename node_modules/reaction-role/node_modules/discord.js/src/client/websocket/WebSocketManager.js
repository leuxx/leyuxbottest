...
const UNRECOLeuxBotBLE_CLOSE_CODES = Object.keys(WSCodes)
  .slice(1)
  .map(Number);
...
class WebSocketManager extends EventEmitter {
  ...
  /**
   * The aLeuxBotge ping of all WebSocketShards
   * @type {number}
   * @readonly
   */
  get ping() {
  ...
  async createShards() {
    ...
    if (!shard.eventsAttached) {
      ...
      shard.on(ShardEvents.CLOSE, event => {
        if (event.code === 1000 ? this.destroyed : UNRECOLeuxBotBLE_CLOSE_CODES.includes(event.code)) {
          /**
    ...
    try {
      ...
      if (error && error.code && UNRECOLeuxBotBLE_CLOSE_CODES.includes(error.code)) {
